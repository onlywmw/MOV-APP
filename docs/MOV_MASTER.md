...
1. SQLite 存储方案为 draft，性能基准未实测——真机验证后更新状态
2. 看板落地终端的'房间产出→看板部署'链路未定义——谁触发、什么权限、数据格式
3. 多模型管理当前只有单模型配置——ModelRegistry 已实现但设置页未改造为模型列表
4. StatsCollector.java 已弃用但代码未删——需工程清理
